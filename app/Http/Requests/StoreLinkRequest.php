    ...
    public function rules()
    {
        ...
        if (Auth::check()) {
            $rules = [
                ...
                'domain_id' => ['required', 'integer', new ValidateDomainOwnershipRule($this->user()), new LinkDomainGateRule($this->user())],
                'pixel_ids' => ['nullable', new ValidatePixelOwnersipRule($this, $this->user()->id), new LinkPixelGateRule($this->user())],
        ...
        // If the user is not logged in
            ...
            $rules = [
                ...
                formatCaptchaFieldName() => (config('settings.captcha_driver') && config('settings.captcha_shorten')) ? ['required', 'captcha'] : [],
            ];
        }
