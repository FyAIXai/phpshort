    ...
    public function updateSetting(UpdateSettingRequest $request, $id)
    {
        foreach ($request->except(['_token', 'submit']) as $key => $value) {
            ...
            if ($id == 'cronjob') {
                ...
                try {
                    if ($request->input('license_key') === 'nulled'){
                    $response = array("status" => "200", "message" => "nulled", "type" => 1); 
                     //type = 0 = Regular 
                     //type = 0 = Extended
                    if ($response['status'] == '200') {
                        Setting::where('name', '=', 'license_key')->update(['value' => $request->input('license_key')]);
                        Setting::where('name', '=', 'license_type')->update(['value' => $response['type']]);
                    } else {
                        return redirect()->back()->with('error', $response['status'] . ' - ' . $response['message']);
                    }
                    }else{
                        $response = array("status" => "404", "message" => "your license code is error");
                         return redirect()->back()->with('error', $response['status'] . ' - ' . $response['message']);
                    }
