    ...
    */
    ...
    public function google(Request $request)
    {
        ...
        try {
            ...
            $request->merge(['email' => $socialUser->email]);
            $request->merge(['name' => mb_substr($socialUser->name, 0, 190)]);
    ...
    public function apple(Request $request)
    {
        ...
        try {
            ...
            $config = \Lcobucci\JWT\Configuration::forSymmetricSigner(
                ...
                \Lcobucci\JWT\Signer\Key\InMemory::plainText(config('settings.auth_apple_private_key')),
            );

            $token = $config->builder()
                ->issuedBy(config('settings.auth_apple_team_id'))
                ->issuedAt($now)
                ->expiresAt($now->addHour())
                ->permittedFor('https://appleid.apple.com')
                ->relatedTo(config('services.apple.client_id'))
                ->withHeader('kid', config('settings.auth_apple_key_id'))
                ->getToken($config->signer(), $config->signingKey());

            config(['services.apple.client_secret' => $token->toString()]);
