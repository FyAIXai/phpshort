...
trait DateRangeTrait
{
    ...
    private function range($from = null, $to = null)
    {
        ...
        try {
            $to = request()->input('to') ? Carbon::createFromFormat('Y-m-d', request()->input('to')) : ($to ? $to->startOfDay() : $now);
        } catch (\Exception $e) {
            $to = $now;
        }

        try {
            $from = request()->input('from') ? Carbon::createFromFormat('Y-m-d', request()->input('from')) : ($from ? $from->startOfDay() : $to);
